...
下列数字全部来自本机实跑，逐请求 JSON、客户端日志与后端日志一并提交在 `benchmark/` 下，完整报告见 [`benchmark/RESULTS.md`](benchmark/RESULTS.md)。测试机 i9-13980HX（24 核 / 32 线程）、32 GB 内存、JDK 25，服务按仓库默认配置启动，未调整 Tomcat 线程数或连接池。

**正确性 —— N 个线程用屏障对齐后，同时抢同一设备的同一时段**

| 并发 N | H2 + LocalReservationLock | MySQL + RedisReservationLock |
| ---: | --- | --- |
| 50 | 1 × `201`，49 × `409` | 1 × `201`，49 × `409` |
| 100 | 1 × `201`，99 × `409` | 1 × `201`，99 × `409` |
| 200 | 1 × `201`，199 × `409` | 1 × `201`，199 × `409` |

每档三轮，两个 profile 合计 **18 轮全部恰好一个成功**，无 worker 异常，无其它状态码。production 后端日志中 `acquired` 与 `released` 各记录 **1792** 次（1790 条进入临界区的已记录请求 + 2 次显式 warmup）。这些是释放**尝试**的日志次数，不能单独证明租约未泄漏，也不能证明 Lua `DEL` 成功；实现现在会根据 Lua 返回值区分成功释放（`1`）与未持有/已过期（`0`）。

**吞吐 —— 每个 worker 独占一台设备，互不冲突，各 500 请求**

| 并发 | H2 + 进程内锁 | MySQL + Redis 锁 |
| ---: | --- | --- |
| 50 | P50 22.8 ms / P99 41.4 ms / **1839 req/s** | P50 275 ms / P99 376 ms / **174 req/s** |
| 100 | P50 44.7 ms / P99 85.1 ms / **1887 req/s** | P50 534 ms / P99 615 ms / **184 req/s** |

**读这组数字有三个坑，比数字本身更值得说明**：

1. **右列不等于"分布式锁的开销"。** 左列是 H2 内存库 + 进程内锁，右列是 WSL 容器里的 MySQL + Redis，两边差的是**整套技术栈**，把差距全部记在 Redis 锁头上是错误归因。要单独量化锁本身的代价，必须在同一个数据库上只切换 `labops.reservation-lock.mode`——这一项尚未测量，是明确的待办而不是已有结论。

2. **Redis 正确性场景的 P99 稳定在 2.0~2.1 秒，是锁等待超时，不是服务卡死。** 默认 `labops.reservation-lock.wait=2s`，抢不到锁的请求等满两秒后返回 `409 RESERVATION_LOCK_TIMEOUT`。因此该列"吞吐随并发上升"（200 ÷ 2.1s ≈ 95 req/s）只是被两秒墙钟钉住的算术结果，**不代表每秒能写入 95 条重叠预约**——真正写入成功的始终只有一条。

3. **同一档位三轮的吞吐是递增的**（H2 在 N=100 时为 501 → 625 → 794 req/s），这是 JVM 的 JIT 预热效应而非数据抖动，因此横向比较只在同一轮次内成立。
