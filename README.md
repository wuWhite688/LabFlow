...
每档三轮，两个 profile 合计 **18 轮全部恰好一个成功**，无 worker 异常，无其它状态码。production 后端日志中 `Redis reservation lock acquired` 与 `released` 各出现 **1792 次，完全配对**——没有一次租约泄漏，Lua 比对 token 的释放路径按预期工作。
