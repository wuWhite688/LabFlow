...
- CSRF：业务接口走 Bearer 头，浏览器不会自动携带；只有 login/refresh/logout 靠 cookie。`SameSite=Lax` 按「站点」判断，兄弟子域（same-site 但不同源）的 POST 仍会带上 cookie，所以另加 Fetch Metadata 校验：BFF 对所有写请求只放行 `Sec-Fetch-Site: same-origin`/`none`（无该头时比对 `Origin` 的 host），后端在这三个接口上再拦一次 `same-site`/`cross-site`。Spring 的 `csrf().disable()` 因此保留
...
- **refresh 响应丢失会被当成 token 重用。** 服务端已提交 A→B 的轮换，但 `Set-Cookie` 没回到浏览器（BFF 到浏览器之间断连，或请求在后端提交后被中止），浏览器手里仍是 A。前端在网络错误时刻意保留会话，所以下一次 refresh 出示 A 而族内 B 仍有效——这与 A 被盗后重放无法区分，于是整族吊销、用户需要重新登录，日志里还会多一条并非攻击的 `refresh token reused`。这是接受的取舍，没有修；常见缓解是给"紧邻的上一代"一个很短的宽限窗口（期间出示它返回当前后继而不是吊销），代价是被盗的上一代在窗口内也能蹭进来
